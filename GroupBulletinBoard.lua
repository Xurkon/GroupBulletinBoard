...
local UPDATE_INTERVAL = 0.2 -- seconds between UI updates
...
-- Optimized: reduced redundant string operations and removed dead code
local splitNoNbCache = {}
function GBB.SplitNoNb(args_msg)
  if type(args_msg) ~= "string" then return {} end
  local cacheKey = args_msg
  if splitNoNbCache[cacheKey] then
    return splitNoNbCache[cacheKey]
  end
  local msgOrg = string.lower(args_msg)
  local msg = string.gsub(msgOrg, "[´`]", "'")
  msg = string.gsub(msg, "''", "'")
  local msg2 = GBB.Tool.stripChars(msg)

  -- Cache common patterns to avoid repeated gsub calls
  local msgOrg_punct_space = string.gsub(msgOrg, "[%p%s%c]", "+")
  local msgOrg_no_digits = string.gsub(msgOrg, "[%p%s%c%d]", "+")
  local msg_punct_space = string.gsub(msg, "[%p%s%c]", "+")
  local msg_no_digits = string.gsub(msg, "[%p%s%c%d]", "+")
  local msg2_punct_space = string.gsub(msg2, "[%p%s%c]", "+")
  local msg2_no_digits = string.gsub(msg2, "[%p%s%c%d]", "+")

  local result = GBB.Tool.iMerge(
    GBB.Tool.Split(msgOrg_punct_space, "+"),
    GBB.Tool.Split(string.gsub(msgOrg, "[%p%c]", ""), " "),
    GBB.Tool.Split(msgOrg_no_digits, "+"),
    GBB.Tool.Split(msg_punct_space, "+"),
    GBB.Tool.Split(string.gsub(msg, "[%p%c]", ""), " "),
    GBB.Tool.Split(msg_no_digits, "+"),
    GBB.Tool.Split(msg2_punct_space, "+"),
    GBB.Tool.Split(string.gsub(msg2, "[%p%c]", ""), " "),
    GBB.Tool.Split(msg2_no_digits, "+")
  )
  ...
  local final = GBB.Tool.iMerge(result, add)
  splitNoNbCache[cacheKey] = final
  return final
end
...
function GBB.OnUpdate(elapsed)
  if GBB.Initalized == true then
    ...
    -- Throttle UI updates using accumulator
    GBB.updateAccumulator = (GBB.updateAccumulator or 0) + elapsed
    if GBB.updateAccumulator >= UPDATE_INTERVAL then
      if GroupBulletinBoardFrame:IsVisible() then
        GBB.UpdateList()
      end
      GBB.updateAccumulator = 0
