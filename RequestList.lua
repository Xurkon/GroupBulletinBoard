...
-- Performance optimization: only re-sort when data changes
...
function GBB.Clear()
  if GBB.ClearNeeded or GBB.ClearTimer < time() then
    ...
    needsSort = true -- List changed, need to re-sort
  end
end
...
function GBB.UpdateList()
  ...
  -- Only sort when data has actually changed (prevents message position jumping)
  if needsSort then
    ...
    for dungeon, id in pairs(dungeonList) do
      ...
      if id == true and dungeon ~= nil then
        ...
        if index == 0 then
          ...
          needsSort = true                   -- New item added, need to re-sort
